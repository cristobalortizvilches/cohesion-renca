...
soci.barrial %>%
  ggplot(aes(x = name, y = sfreq, 
             label = as.character(scales::percent(sfreq, accuracy = .1)))) +
  geom_col(position = 'dodge2') + 
  geom_text(size = 2.5, vjust = 0.5, hjust = -0.3) +
  scale_y_continuous(labels = scales::percent, limits = c(0, 1)) +
...
apoyo.social %>%
  ...
  geom_col(position = 'dodge2') + 
  geom_text(size = 2.5, vjust = 0.3, hjust = -0.3) +
  scale_y_continuous(labels = scales::percent, limits = c(0, 1)) +
...
wsp <- renca %>% 
  ...
  geom_col(position = 'dodge2') + 
  geom_text(size = 2.5, vjust = -1, hjust = 0.5) +
  scale_y_continuous(labels = scales::percent, limits = c(0, 1)) +
...
#remove("fortalezas")
fortaleza1 <- renca %>% 
  dplyr::select(q0026_0001) %>% 
  count(q0026_0001) %>% 
  mutate(freq = (n/sum(n))) %>% 
  ggplot(aes(x = q0026_0001, y = freq, 
             label = as.character(scales::percent(freq, accuracy = .1)))) +
  geom_col(position = 'dodge2') + 
  geom_text(size = 2.5, vjust = 0.5, hjust = -0.1) +
  scale_y_continuous(labels = scales::percent, limits = c(0, 1)) +
  labs(title = "Primera preferencia", x = "", y = "") +
  coord_flip()

fortaleza2 <- renca %>% 
  dplyr::select(q0026_0002) %>% 
  count(q0026_0002) %>% 
  mutate(freq = (n/sum(n))) %>% 
  ggplot(aes(x = q0026_0002, y = freq, 
             label = as.character(scales::percent(freq, accuracy = .1)))) +
  geom_col(position = 'dodge2') + 
  geom_text(size = 2.5, vjust = 0.5, hjust = -0.1) +
  scale_y_continuous(labels = scales::percent, limits = c(0, 1)) +
  labs(title = "Segunda preferencia", x = "", y = "") +
  coord_flip()

fortaleza3 <- renca %>% 
  dplyr::select(q0026_0003) %>% 
  count(q0026_0003) %>% 
  mutate(freq = (n/sum(n))) %>% 
  ggplot(aes(x = q0026_0003, y = freq, 
             label = as.character(scales::percent(freq, accuracy = .1)))) +
  geom_col(position = 'dodge2') + 
  geom_text(size = 2.5, vjust = 0.5, hjust = -0.1) +
  scale_y_continuous(labels = scales::percent, limits = c(0, 1)) +
  labs(title = "Tercera preferencia", x = "", y = "") +
  coord_flip()

grid.arrange(fortaleza1, fortaleza2, fortaleza3, nrow = 2,
             top = textGrob("Señale las 3 principales fortalezas de su barrio y/o comunidad"))
```
## Renca en tres años
